    ...
    HBox {
        VBox {
            Border :title("unit grammar MyGrammar; # (ctrl+enter)"), {
                ...
                END
                MultiLineInput.text($init).cursor(:1row, :4col).on-change: -> $, $text {
                    $error = "";
                    $code = $text
                }
                $code = $init;
                    ...
                    if $code {
                        ...
                        $grammar.^methods.grep({ .WHAT ~~ Regex }).map: -> &rule {
                            &rule.wrap: my method (|c) {
                                ...
                                my $indent = $*INDENT // 0;
                                note "- ENTER: {&rule.name}".indent: $indent;
                                my @parent := @*CHILDREN;
                                {
                                    my $*INDENT = $indent + 4;
                                    my %node;
                                    ...
                                    %node<rule>  = &rule.name;
                                    %node<match> = ?resp;
                                    note "{ resp.raku } -> { ?resp }".indent: $indent;
                                    %node<data>  = (resp || resp.orig.substr: resp.from).Str;

                                    # note "  - PUSH: %node<rule>".indent: $indent;
                                    @parent.push: %node;
                                    # note "  - NOW : { @parent.raku }".indent: $indent;

                                    note "- LEAVE: {&rule.name}".indent: $indent;
                                    return resp
                                }
                            }
                        }
